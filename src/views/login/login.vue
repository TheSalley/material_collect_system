  ...
  loginForm.value.validate(async (valid) => {
    ...
    try {
      ...
      if (res.code === 0) {
        ...
        // 根据角色跳转到对应页面
        const defaultPath = role === "admin" ? "/admin/dashboard" : "/instruction";
        const redirectPath = route.query.redirect;
        const isValidRedirect = typeof redirectPath === "string" && redirectPath && redirectPath !== "/" && redirectPath !== "/login";
        const targetPath = isValidRedirect ? redirectPath : defaultPath;
