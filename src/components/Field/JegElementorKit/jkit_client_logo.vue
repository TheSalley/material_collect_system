  ...
  <div v-if="hasItemsField" class="__field-item">
    ...
    >
      ...
      <div v-show="!collapsedItems.has(index)" class="space-y-3 p-3">
        <div
          v-if="hasItemField(item, 'sg_logo_list_image')"
          class="__field-group"
        >
          <label class="__field-label">
            <el-icon><Picture /></el-icon>
          ...
          <ImageWp
            :model-value="item.sg_logo_list_image"
            :node-id="nodeId"
            :show-size-config="true"
            :width="160"
            @update:model-value="
              (value) => setItemField(index, 'sg_logo_list_image', value)
            "
          />
        </div>
      </div>
    ...
    <el-empty v-if="!items.length" description="暂无客户 Logo 内容" :image-size="60" />
  </div>
</template>

<script setup>
import { computed, reactive } from "vue";
import { ArrowDown, Delete, Picture } from "@element-plus/icons-vue";
...
const hasItemsField = computed(() =>
  Object.prototype.hasOwnProperty.call(props.fields, "sg_logo_list"),
);

const items = computed(() =>
  Array.isArray(props.fields?.sg_logo_list) ? props.fields.sg_logo_list : []
);
...
function emitItems(next) {
  props.onUpdate("sg_logo_list", next);
}
