  ...
  <div v-if="hasItemsField" class="__field-item">
    ...
    >
      ...
      <div v-show="!collapsedItems.has(index)" class="space-y-3 p-3">
        <div
          v-if="hasItemField(item, 'eael_feature_list_title')"
          class="__field-group"
        >
          <label class="__field-label">
          ...
          <el-input
            :model-value="item.eael_feature_list_title"
            placeholder="请输入标题"
            show-word-limit
            @update:model-value="
              (value) => setItemField(index, 'eael_feature_list_title', value)
            "
          />
        </div>

        <div
          v-if="hasItemField(item, 'eael_feature_list_content')"
          class="__field-group"
        >
          <label class="__field-label">
          ...
          <el-input
            :model-value="item.eael_feature_list_content"
            type="textarea"
            :rows="4"
            placeholder="请输入内容"
            show-word-limit
            @update:model-value="
              (value) => setItemField(index, 'eael_feature_list_content', value)
            "
          />
        </div>

        <div
          v-if="hasItemField(item, 'eael_feature_list_icon_type')"
          class="__field-group"
        >
          <label class="__field-label">
          ...
          <el-input
            :model-value="item.eael_feature_list_icon_type"
            placeholder="请输入图标类型"
            @update:model-value="
              (value) =>
                setItemField(index, 'eael_feature_list_icon_type', value)
            "
          />
        </div>
        ...
        <div
          v-if="
            item.eael_feature_list_icon_type === 'image' &&
            hasItemField(item, 'eael_feature_list_img')
          "
          class="__field-group"
        >
          <label class="__field-label">
          ...
          <ImageWp
            :model-value="item.eael_feature_list_img"
            :node-id="nodeId"
            :show-size-config="true"
            :width="160"
            @update:model-value="
              (value) => setItemField(index, 'eael_feature_list_img', value)
            "
          />
        </div>
      </div>
    ...
    <el-empty v-if="!items.length" description="暂无特性列表内容" :image-size="60" />
  </div>
</template>
...
const hasItemsField = computed(() =>
  Object.prototype.hasOwnProperty.call(props.fields, "eael_feature_list")
);

const items = computed(() =>
  Array.isArray(props.fields?.eael_feature_list)
    ? props.fields.eael_feature_list
    : []
);
...
function safeTitle(item, index) {
  const title =
    typeof item?.eael_feature_list_title === "string"
      ? item.eael_feature_list_title.trim()
      : "";

  return title || `特性 ${index + 1}`;
}
...
function emitItems(next) {
  props.onUpdate("eael_feature_list", next);
}
